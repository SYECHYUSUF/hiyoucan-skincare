    ...
    <div class="bg-white p-4 rounded-lg shadow-sm mb-6 flex flex-col md:flex-row gap-4 justify-between">
        <form action="{{ route('admin.users') }}" method="GET" class="flex w-full md:w-auto gap-2">
            <div class="relative w-full md:w-64">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search name or email..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-hiyoucan-500 focus:border-hiyoucan-500 text-sm">
                <svg class="w-5 h-5 text-gray-400 absolute left-3 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
            </div>
            
            <select name="role" class="border border-gray-300 rounded-lg px-4 py-2 text-sm focus:ring-hiyoucan-500 focus:border-hiyoucan-500 bg-white">
                <option value="all">All Roles</option>
                <option value="seller" {{ request('role') == 'seller' ? 'selected' : '' }}>Seller</option>
                <option value="user" {{ request('role') == 'user' ? 'selected' : '' }}>Buyer</option>
            </select>
            
            <button type="submit" class="bg-hiyoucan-700 text-white px-4 py-2 rounded-lg hover:bg-hiyoucan-800 text-sm font-bold transition">Filter</button>
        </form>
    </div>
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="p-4">
                            <div class="flex items-center gap-3">
                                <div class="h-8 w-8 rounded-full bg-hiyoucan-100 flex items-center justify-center text-hiyoucan-700 font-bold text-xs">
                        ...
                        <td class="p-4">
                            <form action="{{ route('admin.users.update-role', $user->id) }}" method="POST" class="flex items-center gap-2">
                                @csrf
                                @method('PATCH')
                                <select name="role" class="text-xs border-gray-300 rounded shadow-sm focus:ring-hiyoucan-500 focus:border-hiyoucan-500 py-1 bg-white cursor-pointer">
                                    <option value="user" {{ $user->role == 'user' ? 'selected' : '' }}>Buyer</option>
                                    <option value="seller" {{ $user->role == 'seller' ? 'selected' : '' }}>Seller</option>
                                </select>
                                <button type="submit" class="text-xs bg-gray-800 text-white px-2 py-1.5 rounded hover:bg-gray-700 transition" title="Save Role">
                        ...
                        <td class="p-4 text-right">
                            <div class="flex justify-end items-center gap-2">
                                <a href="{{ route('admin.users.edit', $user->id) }}" class="text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 p-1.5 rounded transition" title="Edit Info">
                                ...
                                @if($user->role == 'seller')
                                    <form action="{{ route('admin.users.verify', $user->id) }}" method="POST" class="inline-block">
                                        ...
                                        @if(!$user->email_verified_at)
                                            ...
                                            <button type="submit" class="text-green-600 hover:text-green-800 bg-green-50 hover:bg-green-100 p-1.5 rounded transition" title="Approve Seller">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                            </button>
                                        @else
                                            <input type="hidden" name="action" value="reject">
                                            <button type="submit" class="text-orange-500 hover:text-orange-700 bg-orange-50 hover:bg-orange-100 p-1.5 rounded transition" title="Reject/Suspend Seller">
                                ...
                                <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this user permanently? This action cannot be undone.')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 p-1.5 rounded transition" title="Delete User">
