<x-public-layout>
    <div class="pt-32 pb-12 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if(session('error'))
            <div class="bg-red-50 text-red-700 p-4 rounded-xl mb-6 border border-red-100" data-aos="fade-down">{{ session('error') }}</div>
        @endif
        ...
        <nav class="text-sm mb-8 text-gray-500" data-aos="fade-right">
            ...
            <a href="{{ route('shop.index') }}" class="hover:text-hiyoucan-700 transition">Shop</a> <span class="mx-2">/</span>
            <span class="text-gray-900 font-medium">{{ $product->name }}</span>
        </nav>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-20">
            <div class="bg-earth-100 rounded-3xl overflow-hidden aspect-square shadow-sm" data-aos="fade-right">
                <img src="{{ $product->image }}" alt="{{ $product->name }}" class="w-full h-full object-cover hover:scale-105 transition duration-700">
            </div>
            ...
            <div data-aos="fade-left">
                ...
                <div class="flex items-center space-x-4 mb-8">
                    <p class="text-3xl text-hiyoucan-700 font-bold">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                ...
                @else
                    <form action="{{ route('cart.store') }}" method="POST" class="mb-10">
                        ...
                        <div class="flex items-center gap-6">
                            <div class="w-32">
                                <label class="text-xs font-bold text-gray-500 uppercase mb-2 block">Quantity</label>
                                <input type="number" name="quantity" value="1" min="1" class="w-full border-gray-200 bg-gray-50 rounded-xl focus:ring-hiyoucan-500 focus:border-hiyoucan-500 text-center py-3 font-bold">
                            </div>
                            <button type="submit" class="flex-1 bg-hiyoucan-800 text-white px-8 py-4 rounded-xl font-bold hover:bg-hiyoucan-900 transition shadow-lg hover:shadow-hiyoucan-500/30 transform hover:-translate-y-1 flex items-center justify-center gap-3 mt-6">
                ...
                <div class="pt-8 border-t border-gray-100 text-sm text-gray-500 space-y-3">
                    <p class="flex justify-between border-b border-dashed border-gray-200 pb-2">
                        <span>Category</span> 
                        <span class="font-bold text-hiyoucan-700">{{ $product->category ? $product->category->name : 'Uncategorized' }}</span>
                    </p>
                    <p class="flex justify-between border-b border-dashed border-gray-200 pb-2">
                        <span>Stock</span> 
                ...
                <div class="bg-gray-50 p-6 rounded-2xl mb-10 border border-gray-100">
                    ...
                    <form action="{{ route('reviews.store', $product->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="rating" class="border-gray-200 rounded-lg text-sm focus:ring-hiyoucan-500 focus:border-hiyoucan-500">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="comment" rows="3" class="w-full border-gray-200 rounded-xl text-sm focus:ring-hiyoucan-500 focus:border-hiyoucan-500" placeholder="How was the product?"></textarea>
                        </div>
                        <button type="submit" class="bg-hiyoucan-700 text-white px-6 py-2 rounded-lg text-sm font-bold hover:bg-hiyoucan-800 transition">Submit Review</button>
                    </form>
                </div>
                @endif
            @endauth
                ...
                <div class="border-b border-gray-100 pb-8 last:border-0 last:pb-0">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-4">
                            <div class="h-10 w-10 rounded-full bg-hiyoucan-100 flex items-center justify-center text-hiyoucan-700 font-bold text-sm">
        ...
        <div data-aos="fade-up" data-aos-delay="100">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-4 gap-6">
                ...
                <a href="{{ route('shop.show', $related->slug) }}" class="group block bg-white rounded-2xl p-4 shadow-sm hover:shadow-lg transition border border-gray-100">
                    <div class="aspect-square bg-earth-100 rounded-xl overflow-hidden mb-4">
                        <img src="{{ $related->image }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-500">
                    </div>
                    <h4 class="font-bold text-gray-900 truncate group-hover:text-hiyoucan-700 transition">{{ $related->name }}</h4>
                    <p class="text-hiyoucan-700 text-sm font-medium mt-1">Rp {{ number_format($related->price, 0, ',', '.') }}</p>
                </a>
                @endforeach
            </div>
        </div>
